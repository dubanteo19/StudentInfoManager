...
// Định nghĩa class component AddTeacher
class AddTeacher extends Component {
    // Khởi tạo state để lưu trữ tên và email của giảng viên
    state = {
        name: '',
        email: ''
    };
    ...
    handleSubmit = async (event) => {
        ...
        try {
            // Thực hiện yêu cầu POST tới API để thêm giảng viên mới với dữ liệu từ state
            const response = await axios.post('http://localhost:8080/api/teachers', this.state);
            console.log(response.data);  // Log phản hồi từ server
            alert('Giảng viên đã được thêm thành công!');  // Thông báo thành công
            this.setState({ name: '', email: '' });  // Đặt lại state để clear form
        } catch (error) {
            // Xử lý nếu có lỗi trong quá trình gửi yêu cầu
            alert('Lỗi khi thêm giảng viên: ' + error.message);  // Thông báo lỗi
        }
    };
